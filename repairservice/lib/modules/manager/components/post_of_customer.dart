...
class CustomerContainerBuider extends StatelessWidget {
  const CustomerContainerBuider({
    Key key,
    @required this.state,
    @required this.index,
  }) : super(key: key);
      ...
      builder: (context, authstate) {
        return SlidableContainer(
          child: CustomerManagerPostContainer(
            post: state.posts[index],
              ...
              ? () {
                  context.read<PostdetailperfectBloc>().add(
                      PostdetailperfectFetched(
                          postCode: state.posts[index].code, isCustomer: true));
                  Navigator.push(
              ...
              : () {
                  context
                      .read<PostapplyBloc>()
                      .add(PostapplyFetched(state.posts[index].code));
                  Navigator.push(
                      context,
                      SlideFadeRoute(
                          page: PostApplyPage(
          ...
          onPressedDelete: () {
            context
                .read<ManagerBloc>()
                .add(ManagerCustomerDeletePost(state.posts[index].code));
          },
        );
      },
    );
